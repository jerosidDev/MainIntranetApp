...
$(document).ready(function () {
    let tgl;
    let urlJSON = "/PerformanceOverview/RetrieveTBAsInfo";
...
class TBAChartsLoader {
    constructor(TBAdata) {
        this.TBAdata = TBAdata;
        this.AssignedTBA = [];
        this.totalTBA = 0;
        this.GenerateDataTableColChart = () => {
            ...
            for (let k in this.TBAdata) {
                let dataList = this.TBAdata[k];
                let htmlString = this.GenerateToolTipTable(dataList);
                returnedTable.addRow([k, dataList.length, htmlString, ""]);
            }
            return returnedTable;
        };
        this.GenerateToolTipTable = (_dataList) => {
            let bgColor = 'blue';
            let captionTitle = 'to be advised services assigned';
            let cslName = _dataList[0]._contractConsultant.NAME;
            ...
            for (let objData of _dataList) {
                ...
                for (let val in objData) {
                    if (typeof objData[val] === "string")
                        returnedHTML += '<td>' + objData[val] + '</td>';
                }
                returnedHTML += '</tr>';
            }
            returnedHTML += '</table>';
    ...
    ;
    CalculateTBAworkload() {
        let listTBAworkload = [];
        for (let cslName in this.TBAdata) {
            let firstInfo = this.TBAdata[cslName][0];
            let cslWorkload = { INITIALS: firstInfo._contractConsultant.INITIALS, Workload: this.TBAdata[cslName].length };
    ...
    DrawContractCslGauge() {
        ...
        let cslCode = $('#contractCsl').val();
        //     find the consultant object , manage the case non found
        let cslTBA = this.AssignedTBA.filter(function (item) {
...
//# sourceMappingURL=LoadTBAitemsGraphics.js.map